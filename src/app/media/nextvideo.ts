export class NextVideo {
  public static fromUrlUsingDocument(url: string): NextVideo | undefined {
    const links = Array.from(
      document.querySelectorAll('.collection-carousel-media-link a')
    );
  ...
  public static fromElement(element: Element): NextVideo | undefined {
    ...
    const thumbnail: string = img
      .getAttribute('src')!
      .replace(/_[a-zA-Z]+(\.[a-zA-Z]+)$/, '_full$1');
    const seriesAndEpisodeNumber: string = img.getAttribute('alt')!;
    const episode: string = element
      .querySelector('.collection-carousel-overlay-top')!
      .textContent!.trim();
    const title: string = element
      .querySelector('.collection-carousel-overlay-bottom')!
      .textContent!.trim();
    const seriesTitle: string = seriesAndEpisodeNumber.substring(
      0,
